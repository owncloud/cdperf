...
function login_default({ userInfos }) {
  const { credential: userCredential } = userInfos[import_execution.default.vu.idInTest - 1];
  ...
  const [userDisplayName] = queryJson2("displayNamed", userMeResponse?.json(), [userCredential.login]);
  (0, import_k65.check)(userDisplayName, {
