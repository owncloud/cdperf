...
// src/koko/060-create-rename-folder.ts
var create_rename_folder_exports = {};
__export(create_rename_folder_exports, {
  default: () => create_rename_folder_default,
  options: () => options,
  setup: () => setup,
  teardown: () => teardown
});
module.exports = __toCommonJS(create_rename_folder_exports);
...
function create_rename_folder_default({ userInfos }) {
  const { home: userHome, credential: userCredential } = userInfos[import_execution.default.vu.idInTest - 1];
  const userClient = new Client2(settings.baseURL, settings.apiVersion, settings.authAdapter, userCredential);
  (0, import_lodash.times)(settings.folder.rootCount, () => {
    const tree = (0, import_lodash.times)(settings.folder.childCount, () => randomString2());
    tree.reduce((acc, name) => {
