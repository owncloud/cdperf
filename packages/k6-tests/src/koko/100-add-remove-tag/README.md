...
k6 run artifacts/100-add-remove-tag-default-test.js --vus 2 --iterations 5
...
```shell
docker run -e PLATFORM_URL=https://cloud-domain.org:80 -e PLATFORM=ownCloudServer -e AUTH_ADAPTER=basicAuth -e ADMIN_LOGIN=main -e ADMIN_PASSWORD=secret --rm -i grafana/k6 run --vus 2 - < artifacts/100-add-remove-tag-default-test.js
```
