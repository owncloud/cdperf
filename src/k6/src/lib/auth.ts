...
export default class Factory {
    ...
    constructor(account: types.Account) {
        ...
        if (defaults.ENV.CLOUD_OIDC_ENABLED) {
...
class OIDCProvider implements types.AuthProvider {
    private account: types.Account;
    private redirectUri = `${defaults.ENV.CLOUD_OIDC_ISSUER}/oidc-callback.html`;
    private logonUri = `${defaults.ENV.CLOUD_OIDC_ISSUER}/signin/v1/identifier/_/logon`;
    private tokenUrl = `${defaults.ENV.CLOUD_OIDC_ISSUER}/konnect/v1/token`;
    private cache!: {
        validTo: Date;
        token: types.Token;
    };
        ...
        const logonResponse = http.post(
            ...
            }),
            {
                headers: {
                    ...
                    Referer: defaults.ENV.CLOUD_OIDC_ISSUER,
